...
`apps/web/lib/history.ts` provides the `buildTimeline` helper function that turns sparse change-points into chart rows by carrying each series' last known value forward across the union of observation dates, aggregating per bank (max for savings, min for mortgage). The chart is rendered by `RateHistoryChart` (in `apps/web/components/rate-history-chart.tsx`), which displays step lines using Recharts. `SavingsHistorySection` (in `apps/web/components/savings-history-section.tsx`) and `MortgageHistorySection` (in `apps/web/components/mortgage-history-section.tsx`) wrap the chart with bank toggles (and, for mortgages, a currency × segment view selector, since UVR spreads and COP E.A. rates are not comparable).
